...
return (
    <div className="flex w-full h-full fixed">
        <div style={{width: isOpen ? "300px":"65px"}}className="bg-black text-white px-1 py-5 transition-0.5 duration-500">
            <div className="flex items-center p-6">
                <h1 style={{ display: isOpen ? "block" : "none" }} className="text-3xl text-center text-white ">lOiOl</h1>
            ...
            <div style={{ display: isOpen ? "block" : "none" }} className=" py-24 m-3 text-center justify-center transition-all duration-500 before:content-[''] before:fixed before:bottom-64 before:left-0 before:translate-x-1/2 before:w-12 before:h-0.5 before:transition-all before:duration-500 before:bg-blue-500 hover:before:w-36 hover:before:opacity-100">
            {
                user.map((user, page) => (
                    <NavLink to={user.way} keys={page} className="flex text-gray-200 rounded-md h-7 gap-3">
